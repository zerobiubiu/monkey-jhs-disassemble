...
### 3.1 150–375 行带：Round 4 拆分目标 barrel 导演级核查（只读）

§3 覆盖 ≥290 行；本节补查 150–375 行带中**被 Round 4 原子化触碰的 barrel**（即残留/欠拆分最可能藏身之处），以 import 图为透镜亲自核查，确认拆分已落地、barrel 为协调器：

| 文件 | 行数 | 观察到的证据 | 判定 |
|------|------|-------------|------|
| other-site-plugin.tsx | 375 | L45-52 import osp-enabled-sites / osp-filter-bar / osp-preload / osp-probe + 4 组件；类 = 薄委托协调器 | 协调器 |
| vlt-filter-bar.ts | 311 | L11-18 import 并 L20 再导出 vlt-filter-apply 的 apply/collect 函数；本文件仅余筛选栏 UI 构建（buildFilterBar/createFilterChip/refreshChips/...） | 内聚（UI 半，apply 已拆） |
| auto-page-plugin.ts | 304 | L32-43 import ap-fetch.loadNextPage + ap-scroll 全 9 函数；类 = 薄委托协调器 | 协调器 |
| list-waterfall-plugin.ts | 279 | L48-49 import lw-fetch（loadNextPage/findNextUrl/...）+ lw-scroll（checkLoad/updateCurrentPageFromScroll）；barrel 仅余 getInitialPage/isEnabled + 类壳 | 协调器 |
| status-tag-filter-plugin.ts | 235 | L65-66 import stf-apply.applyFilter + stf-ui.doBuild/findMountTarget；collect 已完全移出（见下注） | 协调器 |
| lrs-toolbar.ts | 213 | L18-24 import lrs-filter 的 applySortAndFilter/saveFilter*/countFilterStats/createFilterChip 并 L27 再导出；barrel 仅余状态持久化 + 工具栏构建/刷新 | 协调器 |

注（两处「顶层未 import 某同级模块」非残留，而是更整洁的设计）：status-tag-filter 的 barrel 顶层不 import `stf-collect`、lrs-toolbar 顶层不 import `lrs-sort`——因 collect/排序函数已被**完全移出** barrel，改由其下一级消费者（stf-ui.doBuild / lrs-filter.applySortAndFilter）内部 import 调用；barrel 自身不再直接调用它们，故无需顶层 import。这恰是「helper 彻底离开 barrel」的证据，而非欠拆分。

覆盖范围说明（诚实边界）：导演级 import 图核查至此覆盖 **≥290 行全带（§3，14 文件）+ 150–375 行的 Round 4 拆分目标 barrel（本节，6 文件）**。150–300 行带中**非** Round 4 拆分目标的单插件文件（如 visit-history / dpb-want-watched / magnet-hub / fc2 / api / missav-quick-copy / jsx-to-string / data-import-export / car-list-ops / list-page-button 等）仍为工作级审计判定——其内聚性由「单一 BasePlugin 子类 + 单一职责」结构保证（cohesion 准则下小混合文件本即 L），复用维度由非按文件大小门控的跨文件 dup 扫描覆盖；导演级未逐文件复核该子集。
...
- 本文档提交：纯文档，不递增版本（保持 1.28.19）；后续以纯文档提交补入 §3.1（150–375 行带导演级核查），仍不递增版本。
