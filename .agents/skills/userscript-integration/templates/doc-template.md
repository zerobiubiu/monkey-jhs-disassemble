...
| 数据源 | <结论> | <依据：是否复用 storageManager / 保留原生 IDB> |
| 事件源 | <结论> | <依据：是否监听本项目已有事件 / 自建> |
| GM_xmlhttpRequest | <结论> | <依据：grant 是否已含；是否复用 gmRequest> |
| GM_addStyle | ❌ 改走 initCss | 项目既定模式 |
| GM_registerMenuCommand | <结论> | <依据：原脚本菜单命令数 + grant 是否需补> |
