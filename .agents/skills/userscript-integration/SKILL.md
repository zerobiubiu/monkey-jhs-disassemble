...
**不要直接开写**。先对原脚本做 6 维度调研，输出一张决策表。这是 doc/25 §1.3 + doc/37 冲突排查的模式。
...
| **主项目冲突** | **原脚本操作的选择器/DOM 容器/data 属性/功能，是否与主项目已有插件重叠？** | **grep 原脚本的选择器/属性名到 `src/` 搜索；重点查 `.movie-list`/`.toolbar`/`data-*-index`/`MutationObserver`/排序/过滤等；见第 1.5 步** |
...
### 第 1.5 步：主项目冲突排查（关键，不可跳过）

独立脚本原本独立运行，集成后与主项目 27 个插件同处一个 userscript 上下文，
**可能产生深度耦合冲突**。doc/37 的 PageSortPlugin 案例（与 jhs 排序系统双重排序 +
两个 MutationObserver 互相触发死循环 + autoPage 瀑布流打乱）是典型教训。

**排查方法**：

1. **提取原脚本的关键操作符**：grep 原脚本的选择器、data 属性名、DOM 容器名、
   MutationObserver 监听目标、localStorage 键名。
2. **到 `src/` 全项目搜索**：用 grep 搜这些关键操作符，找出主项目中操作同一目标的插件。
3. **判定冲突类型**：

| 冲突类型 | 表现 | 典型案例 | 协调策略 |
|----------|------|----------|----------|
| 双重功能重叠 | 两个插件做同一件事（排序/过滤/显隐） | PageSort vs `ListPageButtonPlugin.sortItems` | 排序互斥：点击本插件按钮清除对方状态，反之亦然 |
| MutationObserver 互相触发 | 两个 observer 监听同一 DOM，互相 append 触发对方 | PageSort sortGuard vs `ListPagePlugin.checkDom` observer | applySort 内 disconnect/reconnect 自身 observer；或对方操作时本插件 observer 不反应（通过状态守卫） |
| DOM 容器共享冲突 | 多个插件操作同一容器子项 | PageSort/`AutoPagePlugin`/`ListPagePlugin` 都操作 `.movie-list .item` | 加 autoPage/isListPage 守卫，在对方生效时本插件不注入或不排序 |
| data 属性冲突 | 两套 data 属性标记原始位置 | `data-original-index` vs `data-sort-original-index` | 复用主项目已有的 data 属性，不另建 |
| 事件监听冲突 | 同一元素同一事件多个监听器 | keydown 方向键 vs `Hotkey.handleKeydown` | 加 `isTyping()` 焦点守卫，或检查 `Hotkey` 是否已处理 |

4. **输出冲突排查表**：在文档 §1.3 之后追加 §1.4「主项目冲突排查」，列出每个冲突
   及协调策略。若无冲突，明确标注“无冲突，独立运行”。

**参考样本**：`doc/37-page-sort-jhs-coordination.md` 是冲突排查 + 协调的完整记录。
...
若第 1.5 步发现冲突，在编码时落实协调策略。原则：**本插件单方面适配，零侵入主项目已定稿插件**（避免修改已定稿逻辑引入新风险）。

常见协调模式：

1. **autoPage/isListPage 守卫**：`handle()` 首行检查 `window.isListPage` / `storageManager.getSetting('autoPage')`，与主项目插件一致地跳过。
2. **复用 data 属性**：用主项目已有的 `data-original-index` 等，不另建；仅当未标记时才打标（`if (!el.hasAttribute('data-x')) el.setAttribute(...)`）。
3. **排序/功能互斥**：本插件激活时清除对方的状态（如 `localStorage.removeItem('jhs_sortMethod')`）；监听对方按钮 click 清除本插件状态。
4. **MutationObserver 暂停**：`applySort` 前临时 `disconnect()` 自身 observer，排序完成 + `takeRecords()` 后 `reconnect`（`observe()` 重建），避免自身 append 触发守卫。
5. **事件委托 + e.currentTarget**：jQuery `function(e){ $(this) }` 改箭头函数 + `e.currentTarget`，保留 `this` 为类实例，用 `getBean()` 协调其他插件。

参考 `src/plugins/page-sort-plugin.ts` + `doc/37` 的完整协调实现。
...
### 1.3 可行性分析
（6 维度决策表：站点/GM_API/数据源/事件源/网络请求/主项目冲突）
### 1.4 主项目冲突排查
（冲突类型 | 表现 | 协调策略 表格；无冲突则标注“无冲突，独立运行”）
...
8. **与主项目协调**：独立脚本集成后与主项目插件同处一个上下文，可能产生深度耦合冲突（双重排序/MutationObserver 互相触发/DOM 容器共享/data 属性冲突）。**第 1.5 步冲突排查不可跳过**，发现的冲突必须在 §1.4 列出协调策略并在 §3.5 落实。原则：本插件单方面适配，零侵入主项目已定稿插件。
...
2. 6 维度调研：javdb only / 无 GM_API / 无数据源 / 无事件源 / 无网络请求 / **主项目冲突：操作 `pagination-previous/next` 选择器，grep `src/` 确认无其他插件操作此选择器 → 无冲突**
...
6. 文档：`doc/34-key-page-turning-integration.md`，§1.4 标注“无冲突，独立运行”
7. 验证：`bunx tsc -b && bunx vite build`
8. 回填 §4 验证记录，状态 ✅已执行

**复杂冲突案例**：用户说"把 `archetype/pageSort.user.js` 集成进来"时，agent 应：

1. 读 `archetype/pageSort.user.js`（190 行，内容排序）
2. 6 维度调研：javdb only / 无 GM_API / 无数据源 / 无事件源 / 无网络请求 / **主项目冲突：操作 `.movie-list .item` 排序 + MutationObserver 监听 childList → grep `src/` 发现 `ListPageButtonPlugin.sortItems` 也在排序 + `ListPagePlugin.checkDom` 也有 MutationObserver + `AutoPagePlugin` 瀑布流 append `.movie-list` → 三重冲突**
3. 冲突协调策略（§1.4）：autoPage/isListPage 守卫 + 复用 `data-original-index` + 排序互斥（清除 `jhs_sortMethod` / 监听 `#sort-toggle-btn`）+ sortGuard disconnect/reconnect
4. 拆分决策：190 行单文件 + CSS
5. 编码：落实协调策略（§3.5）
6. 文档：`doc/36` 集成 + `doc/37` 协调优化（两份文档）
7. 验证 + 回填
...
- **与主项目冲突**：第 1.5 步发现的冲突必须在 §1.4 列出协调策略，在 §3.5 落实编码。原则：本插件单方面适配，零侵入主项目已定稿插件。复杂冲突可能需两份文档（集成 doc/NN + 协调优化 doc/NN+1，参考 doc/36+37 模式）
