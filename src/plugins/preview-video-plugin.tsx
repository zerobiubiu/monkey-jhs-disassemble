 ...
 */
const selectAvailableVideoQuality = (
    available: string[],
    desired: string,
): string | null => {
...
class DmmPreviewVideoResolver {
    ...
    _checkCache(): QualityVideoMap | null {
        ...
        const cache: Record<string, QualityVideoMap> = rawCache
            ? JSON.parse(rawCache)
            : {};
...
export class PreviewVideoPlugin extends BasePlugin {
    ...
    async initDmm(): Promise<void> {
        try {
            const videoMap = await fetchDmmPreviewVideo(
                this.getPageInfo().carNum as string,
                false,
            );
            if (!videoMap) {
                return;
