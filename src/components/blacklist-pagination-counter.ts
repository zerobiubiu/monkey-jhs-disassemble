/**
 * BlacklistPaginationCounter —— 黑名单表格分页计数器 HTML 字符串组件。
 *
 ...
 * 实体、末尾两个空格（`...currentCarCount}  <span` 原脚本如此）原样不动，
 * 与原模板零偏差。
 *
 * 渲染方式：普通函数返回 HTML 字符串，供 loadTableData 中 Tabulator
 * paginationCounter 回调 `return BlacklistPaginationCounter({...})` 消费。
...
/**
 * 渲染黑名单表格分页计数器的 HTML 字符串。
 * @param props.actorCount 当前页演员数
 * @param props.currentCarCount 番号总数
 * @returns 统计文案 + checkBlacklistMsg 占位 span，供 paginationCounter 返回。
 */
export function BlacklistPaginationCounter({
    actorCount,
    currentCarCount,
}: BlacklistPaginationCounterProps): string {
    return `演员: ${actorCount} &nbsp;&nbsp;&nbsp;番号总数: ${currentCarCount}  <span id="checkBlacklistMsg" style="margin-left: 10px"></span>`;
}
