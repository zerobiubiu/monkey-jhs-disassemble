/**
 * RelatedHeader —— 相关合集区头部（分隔线 + 折叠/展开按钮）HTML 字符串组件。
 *
 * 为 RelatedPlugin.showRelated 提供：左右渐变分隔线 + "📁 相关合集" + 折叠/展开按钮
 * （含 toggle-text / toggle-icon），由 `target.append(html)` 消费。
 * isExpanded === YES 时显示"折叠▲"，否则"展开▼"。
 *
 * 结构对称 ReviewHeader（doc/13 已提取），文案改为"相关合集"并移除评论专属 data-tip。
 *
