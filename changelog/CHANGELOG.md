...
- **HTML 与外链边界**（doc/129）：`jsxToString` 统一转义文本和属性；评论、日志、
  tooltip、热播、翻译、FC2、截图墙和磁链结果不再直接注入远程 HTML；JavDB、
  Wikipedia、JavStore、123AV 与第三方站点链接增加 HTTPS/主机/同源验证。
- **用户脚本权限范围**（doc/129）：GM 高权限服务只挂 userscript 沙箱；站点匹配
  收紧为 JavDB/MissAV 官方精确域名并启用 `noframes`。任意 WebDAV 地址仍需
  `@connect *`，未改变既有云备份能力。

### 修复

- **安全导入与最小回滚**（doc/130）：主备份使用键/schema allowlist、先导入
  IndexedDB 后恢复附加存储；旧备份不再删除缺失的新插件键，失败仅条件回滚本次
  触及且尚未被并发更新的键。
- **MissAV 空导入保护**（doc/130）：错误 JSON、空对象和损坏列存不再降级为 0 条
  后清库；结构合法的空清单也必须二次确认，底层接口默认拒绝未确认空导入。
- **跨站修订一致性**（doc/130）：GM 增量按番号压缩并在压缩后仍超阈值时以 gzip 全量
  收敛，配合单调 revision；全量包增加 ready/count 校验；cars 或 revision 任一步
  失败均恢复写前数据和旧水位，避免旧载荷回退/复活状态。
- **缓存与设置面板**（doc/130）：修复番号、收藏演员、黑名单和设置缓存失效遗漏；
  修复切换缓存/预加载面板时 DOM `this` 导致统计方法不可调用。

### 优化

- **IDB 对账**（doc/130）：全量先写入、核对条数再删除，失败恢复原始快照；查询失败
  不再被当成空结果移除标签；大于 500 条的增量自动改发压缩全量。
- **运行时容错**（doc/129）：启动存储维护按依赖顺序执行、失败后停止后续迁移但继续
  启动插件；详情页可选依赖增加 15 秒等待上限和回退导航；手动同步反馈真实发布结果。

本版本只包含本地代码与构建产物变更，没有自动部署，也没有修改线上或用户现有数据。
